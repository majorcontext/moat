...
- **Isolated Execution** - Each agent runs in its own container with workspace mounting
...
To use `moat grant github`, you need a GitHub OAuth App configured for device flow:

1. Go to [GitHub Developer Settings](https://github.com/settings/developers)
2. Click **New OAuth App**
3. Fill in the details:
   - **Application name:** Moat (or your preferred name)
   - **Homepage URL:** `https://github.com/andybons/moat`
   - **Authorization callback URL:** `https://github.com/andybons/moat` (not used for device flow)
...
```bash
export MOAT_GITHUB_CLIENT_ID="your-client-id-here"
```
...
This example runs a container that calls the GitHub API. The container has no access to your token—Moat injects it at the network layer.
...
The `curl` command has no authentication flags—Moat injected the `Authorization` header automatically.
...
The network firewall is enforced via the credential-injecting proxy that containers already use for authentication. When a request is blocked, the agent receives a `407 Proxy Authentication Required` response with an `X-Moat-Blocked: network-policy` header and an actionable error message.
...
```bash
$ moat run -- curl -v https://blocked.example.com
...
|----------|--------|---------|
| 1 | `--name` flag | `moat run --name myapp` |
...
curl --cacert ~/.moat/proxy/ca/ca.crt https://web.myapp.localhost:8080
```

The proxy binds to port 8080 by default. Configure globally in `~/.moat/config.yaml`:
...
Or via environment variable: `MOAT_PROXY_PORT=9000`
...
```bash
moat run [path] [flags] [-- command]

# Examples
moat run                                      # Run from current directory
moat run ./my-project                         # Run from specific directory
moat run --name myapp                         # Run with specific name
moat run --grant github                       # Run with GitHub credentials
moat run -e DEBUG=true                        # Run with environment variable
moat run -- npm test                          # Run with custom command
```
...
Store credentials for injection into moat runs.

```bash
moat grant <provider>[:<scopes>]

# Examples
moat grant github              # Grant with default scopes
moat grant github:repo         # Grant with specific scope
moat grant github:repo,user    # Grant with multiple scopes
```
...
```bash
moat logs [run-id] [flags]

# Examples
moat logs                      # Logs from most recent run
moat logs run-abc123           # Logs from specific run
...
```bash
moat trace [run-id] [flags]

# Examples
moat trace                     # Traces from most recent run
moat trace run-abc123          # Traces from specific run
...
On first start, the proxy generates a CA certificate at `~/.moat/proxy/ca/ca.crt`. Trust this certificate to avoid browser warnings (see "Accessing Services" above).
...
```bash
moat audit [run-id] [flags]

# Examples
moat audit                           # Audit most recent run
moat audit run-abc123                # Audit specific run
moat audit --export proof.json       # Export proof bundle for offline verification
moat audit run-abc123 -e audit.json  # Export specific run's proof bundle
...
Moat automatically detects and uses the best available container runtime:
...
When you run `moat grant github`, your GitHub token is stored securely. During runs with `--grant github`, Moat:
...
Moat can pull secrets from external backends and inject them as environment variables. Unlike credential injection (which happens at the network layer), secrets are resolved before the container starts and passed as environment variables.
...
The format is `op://vault/item/field`. When you run `moat run`, Moat resolves each secret reference and injects the value as an environment variable.
...
**Note:** Secrets providers delegate to external CLI tools rather than bundling SDKs. This keeps the Moat binary small and leverages existing authentication (SSO, credential files, etc.). Native SDK integration may be added in the future for performance-sensitive use cases.

### Image Selection

Moat selects the container base image based on `dependencies` in `agent.yaml`:
...
- **Logs** - Timestamped container output (`~/.moat/runs/<id>/logs.jsonl`)
- **Network** - All HTTP/HTTPS requests (`~/.moat/runs/<id>/network.jsonl`)
- **Traces** - OpenTelemetry-compatible spans (`~/.moat/runs/<id>/traces.jsonl`)
- **Secrets** - Which secrets were resolved (names/backends only, never values) (`~/.moat/runs/<id>/secrets.jsonl`)
- **Metadata** - Run configuration (`~/.moat/runs/<id>/metadata.json`)
- **Audit Log** - Tamper-proof audit database (`~/.moat/runs/<id>/audit.db`)

### Tamper-Proof Audit Logging

Moat provides cryptographic verification that audit logs haven't been tampered with:
...
```bash
moat audit run-abc123 --export proof.json
moat verify-bundle proof.json
```
