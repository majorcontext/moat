...
- Fix `ctrl+/` doing nothing under Codex — Moat's escape prefix matched a single byte, `0x1f`, the legacy encoding of a chord that is ambiguous with `ctrl+_`. Codex enables the kitty keyboard protocol at startup (Claude Code does not), which asks the terminal to disambiguate, so the terminal reports `ctrl+/` as `CSI 47;5u` and Moat never saw it. Every command behind the prefix was unreachable under Codex, including the `ctrl+/ d` TUI dump that exists to debug this class of problem. Moat now recognizes the CSI-u encoding alongside the raw byte, ignores the key-up and autorepeat events that accompany a real chord, and leaves every other kitty-encoded key — including `ctrl+c` — untouched for the agent. ([#450](https://github.com/majorcontext/moat/pull/450))
- Fix Codex's prompt area smearing, the cursor landing in unexpected places, and content shifting wholesale on every update — Moat replaced every scroll region the child requested with its own, so the footer row could not be scrolled away ([#349](https://github.com/majorcontext/moat/pull/349)). That assumes the child never needs a region of its own. Codex renders inline, never entering the alternate screen, and uses scroll regions as a drawing primitive: restrict to a band, reverse-index to open space inside it, reset, redraw. With the band replaced by the full content area, those reverse-indexes scrolled the entire screen. Moat now clamps the child's region instead of discarding it — the requested band is forwarded with its bottom margin held above the footer row, so Moat still owns that row while the agent keeps its band. ([#450](https://github.com/majorcontext/moat/pull/450))
- Fix the status footer disappearing until the agent stopped rendering — "erase from the cursor to the end of the display" (`CSI J`) reaches past the child's content area into the footer row, and Moat only repaired the footer on a redraw debounced behind 50ms of quiet. Under continuous rendering that quiet never came. The footer is now repainted immediately after the erase, with the cursor preserved. ([#450](https://github.com/majorcontext/moat/pull/450))
...
- Fix `moat grant providers` listing grants that do not exist — previously the table showed `anthropic` twice (the `claude` OAuth provider was displayed under an `anthropic` alias while the real `anthropic` API-key provider registered under the same name), listed `pi` even though Pi has no credential of its own and `moat grant pi` always errors, and left the description column blank for `meta`, `oauth`, and `pi`. Every listed name is now a name `moat grant <name>` accepts, `claude` and `anthropic` appear as the separate grants they are, and every row carries a description. ([#447](https://github.com/majorcontext/moat/pull/447))
- Fix `pip:` dependencies failing the image build with `externally-managed-environment` — previously, when Python was not the sole runtime (e.g. `python` alongside `node`, where the base image is `debian:bookworm-slim` rather than `python:X-slim`), Python came from Debian's `python3` package, which ships the PEP 668 `EXTERNALLY-MANAGED` marker. Every `pip install` then failed, so a config with `pip:numpy` could not build. That marker is now removed when Moat installs Python via apt, which also unblocks `pip install` and `uv pip install --system` in build hooks and in the running container. ([#445](https://github.com/majorcontext/moat/pull/445))
