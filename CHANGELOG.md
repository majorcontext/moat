...
- **Copilot CLI settings passthrough** — `moat copilot` now carries over user preferences from the host's `~/.copilot/settings.json` (contextTier, effortLevel, footer, includeCoAuthoredBy, model, mouse, subagents, tabs, theme). An optional `~/.moat/copilot/settings.json` provides moat-specific overrides that win over host settings. Settings that execute commands (`statusLine`) are only allowed from the moat override file. CLI flags and `moat.yaml` fields take precedence over settings.json values. ([#438](https://github.com/majorcontext/moat/pull/438))
