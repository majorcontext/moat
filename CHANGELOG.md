...
- Fix potential `routes.json` corruption on a crash — the hostname-routing table was written non-atomically (truncate-then-write), so a crash mid-write could leave a truncated file that every routing proxy then failed to parse. It is now written atomically via a temp file and rename. ([#412](https://github.com/majorcontext/moat/pull/412))
- Fix a data race on a run's provider metadata — provider stopped-hooks wrote `ProviderMeta` while metadata-save read it concurrently (from the container-exit monitor and `moat stop`) without synchronization. Both paths now hold the run's state lock. ([#412](https://github.com/majorcontext/moat/pull/412))
