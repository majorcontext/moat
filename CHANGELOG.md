...
- Fix `pnpm install` (and other corepack-driven setup) intermittently failing in `pre_run` hooks with a connect timeout (`UND_ERR_CONNECT_TIMEOUT`) — previously, when corepack had to download the pinned package-manager version (e.g. a `packageManager: pnpm@x.y.z` not already baked into the image), it connected **directly** to the npm registry, because Node 24's built-in fetch ignores `HTTP(S)_PROXY` unless opted in. moat's egress firewall drops direct connections (all egress must traverse the proxy), so the download timed out and aborted the run before the agent started. Whether it failed depended on a race between the firewall install and the hook, so the same image could work one run and fail the next — switching Rancher Desktop to the VZ backend (slower container cold-start) made it reliably fail. `moat-init.sh` now sets `NODE_USE_ENV_PROXY=1` for the `pre_run` hook when a proxy is configured, so corepack routes through the moat proxy instead of attempting blocked direct egress. Scoped to the hook; the agent's own proxy handling is unchanged. Takes effect on the next image rebuild (the embedded init script is part of the image hash, so the rebuild is automatic). ([#401](https://github.com/majorcontext/moat/pull/401))
