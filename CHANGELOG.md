...
- **`gitleaks` dependency** — add the [gitleaks](https://github.com/gitleaks/gitleaks) secret scanner to `dependencies` to make it available in the container (e.g. for a pre-commit hook). Installs as a prebuilt release binary with no image rebuild cost beyond its own layer. See [Dependencies](https://majorcontext.com/moat/reference/dependencies). ([#442](https://github.com/majorcontext/moat/pull/442))
