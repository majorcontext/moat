...
Adds HTTP request-body inspection to Keep policies. File- and pack-based `network.keep_policy` rules can now match on the parsed JSON request body, so policies can enforce content-based rules (e.g. block requests whose body carries a secret) instead of host/method/path alone. Also adds native in-VM volume backing for `volumes:` (`type: volume`), which keeps `node_modules` and pnpm stores off virtiofs on macOS — faster, and free of the pnpm v11 SQLite-WAL deadlock.

### Added

- **Native volume backing for `volumes:`** — a volume entry can set `type: volume` to use an in-VM Docker named volume (`moat_<agent>_<name>`) instead of a host bind mount, keeping `node_modules` and pnpm stores off virtiofs. This is faster for many-small-file workloads and avoids the pnpm v11 SQLite-WAL index deadlock that occurs when the store lives on a virtiofs mount (macOS Docker Desktop / Rancher Desktop). The default is unchanged (`bind`), so existing configs are unaffected. Docker runtime only; the Apple container runtime rejects `type: volume`. See [Volumes](https://majorcontext.com/moat/reference/moat-yaml). ([#NNN](https://github.com/majorcontext/moat/pull/NNN))
