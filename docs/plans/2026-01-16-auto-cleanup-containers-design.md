...
Containers accumulate after runs complete, consuming significant disk space. Users must manually run `moat destroy <id>` to clean up. In practice, this is forgotten, leading to 70GB+ of orphaned containers.
...
Containers are removed immediately after a run completes (via `Wait()` or `Stop()`). Logs and network data are already captured to `~/.moat/runs/<id>/`, so no observability is lost.
...
When `--keep` is used, the container remains and can be cleaned up later with `moat destroy <id>`.
