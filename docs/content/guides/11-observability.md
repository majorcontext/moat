...
| `serial-<name>.capture` | Serial payload bytes, when the device is configured `record: full` |
...
counters. `kind` is one of `attach`, `detach`, `settings`, `modem`, `break`,
`purge`, `error`, `conflict`.
...
`moat audit <run-id>` shows the audited subset — attach, detach, and errors,
with the device's USB identity — in the tamper-evident chain. `devices.jsonl`
is a plain append-only log, not part of the chain.
