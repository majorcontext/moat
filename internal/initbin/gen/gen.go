// Command gen cross-compiles cmd/moat-init into the embed/ blobs and
// refreshes checksums.txt. It is invoked by `go generate ./internal/initbin`
// from the initbin package directory (go generate sets the working directory
// to the directory of the file containing the directive).
//
...
const target = "github.com/majorcontext/moat/cmd/moat-init"

func main() {
	arches := []string{"amd64", "arm64"}
	sums := ""
	for _, arch := range arches {
		out := filepath.Join("embed", "moat-init-linux-"+arch)
		...
		if err := cmd.Run(); err != nil {
			fmt.Fprintf(os.Stderr, "initbin gen: building %s for %s: %v\n", target, arch, err)
			os.Exit(1)
		}
		data, err := os.ReadFile(out)
		if err != nil {
			fmt.Fprintf(os.Stderr, "initbin gen: reading %s: %v\n", out, err)
			os.Exit(1)
		}
	...
	if err := os.WriteFile("checksums.txt", []byte(sums), 0o644); err != nil {
		fmt.Fprintf(os.Stderr, "initbin gen: writing checksums.txt: %v\n", err)
		os.Exit(1)
	}
}
