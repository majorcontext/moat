...
func ImageTag(deps []Dependency, opts *ImageSpec) string {
	...
	// When the moat-init entrypoint is used, hash the embedded binary so that
	// a rebuilt entrypoint invalidates cached images. Without this, users on
	// runtimes without --add-host (Apple) can end up running stale images
	// that lack critical initialization logic. Mirror the conditions in
	// needsInit() plus any dep-driven DockerMode.
...
// entrypoint: the SHA-256 of the embedded Go binary, so a rebuilt entrypoint
// re-keys freshly built images.
//
// The "moat-init-v3" label is a deliberate salt bump so a warm-cache lookup
// cannot resolve to an image cached under an earlier scheme (v1 hashed the
// shell script; v2 hashed the script + dispatcher + binary during the
// dual-ship window). This re-keys the `moat run` build/lookup path only — a
// workflow pinning a concrete moat/run:<hash> tag still resolves the old
// image and must re-tag/rebuild.
func initHashComponent() string {
	h := sha256.Sum256(initbin.Binary())
	return "moat-init-v3:" + hex.EncodeToString(h[:])[:8]
}
