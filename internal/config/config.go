...
type Config struct {
	...
	Agent        string            `yaml:"agent" doc:"one of: claude, claude-code, codex, copilot, gemini, pi (the agent aliases openai and google are accepted too). Omit unless the project pins a specific agent."`
...
// - Otherwise, enable sync_logs if "codex" or "openai" is in grants
func (c *Config) ShouldSyncCodexLogs() bool {
	if c.Codex.SyncLogs != nil {
		return *c.Codex.SyncLogs
	}
	// Default: enable for either subscription or API-key Codex auth.
	for _, grant := range c.Grants {
		if grant == "codex" || strings.HasPrefix(grant, "codex:") || grant == "openai" || strings.HasPrefix(grant, "openai:") {
			return true
		}
	}
	return false
